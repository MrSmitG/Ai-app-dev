...
export const BUNDLE_CATALOG = [
  ...
  },
  {
    ...
    blurb: "Enables the Agent tab with an API key (Options → Agent). Cloud or local Cursor runtime.",
    category: "agent",
    exclusiveGroup: "agent-runtime",
    ramHintGb: 8,
    settings: { cursorRuntime: "local", agentMode: "forge", cursorModel: "composer-2.5" },
    skillId: "craftsman",
  },
  {
    id: "local-vision-agent",
    name: "Local vision agent",
    tagline: "No API key. Vision → LLM think → act, with a tracked log.",
    blurb: "Lightweight loop: a computer-vision model turns images into text cards, the local LLM plans from your prompt, then acts in the workspace. Agentic logs keep its place in the workflow. Uses Ollama moondream/llava if present, otherwise your loaded GGUF.",
    category: "agent",
    exclusiveGroup: "agent-runtime",
    ramHintGb: 8,
    settings: {
      cursorRuntime: "local-vision",
      agentMode: "local",
      visionEnabled: true,
  ...
  },
  {
    ...
    tagline: "Planner, memory, tool bus, critic, HITL, and budgets.",
    blurb: "Turns on the full Observe → Reason → Choose → Execute → Verify loop with Harbor RAG, scoped memory, rate limits, and Critic evaluation. Start on the Agent tab → Framework.",
    category: "agent",
    exclusiveGroup: "agent-runtime",
    ramHintGb: 8,
    settings: {
